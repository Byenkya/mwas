...
from mwas.models import (User,Administrator,grow,Farmer,Farmer_crops,Customer,Crops_of_interest,Compliant,
                         Post,Order,Marketing_stat_rp,Marketing_stat_wp,Advert,Market_available)
...
class AdvertsView(BaseView):
    ...
    def index(self):
        ...
        if data:
            flash('This a web page that shows details about Adverts sent by users(Farmers) of the System','info')
...
class AdvertCustomerView(BaseView):
    ...
    def index(self):
        ...
        if data:
            flash('This a web page that shows details about Adverts sent by users(Wholesalers, Retailers etc.) of the System','info')
...
## avery protected route for only system Admins
...
def MWASdataxxcAdmin():
    ...
    if form.validate_on_submit():
        ...
        admin = Administrator(first_name=form.first_name.data,last_name=form.last_name.data,username=form.username.data,
...
def register_farmer():
    ...
    if form.validate_on_submit():
        ...
        if form.sigin.data== "Farmer":
            user = User(username=form.username.data, email=form.email.data,sign_in_as=form.sigin.data,password=hashed_password)
            farmer = Farmer(first_name=form.first_name.data,last_name=form.last_name.data,username=form.username.data,email=form.email.data,
                            phone_number=form.pnumber.data,physical_address=form.physical_add.data)
            ...
            flash("Your Account has been Created! You are now able to log in", "success")
             ...
             user = User(username=form.username.data, email=form.email.data,sign_in_as=form.sigin.data,password=hashed_password)
             customer = Customer(first_name=form.first_name.data,last_name=form.last_name.data,username=form.username.data,email=form.email.data,
                            phone_number=form.pnumber.data,physical_address=form.physical_add.data)
...
def charts():
    ...
    if form.validate_on_submit():
        ...
        return render_template('charts.html',title="MWAS charts",query_list=query_list, all_items=all_items)
...
def complaint():
    form = ComplaintForm()
    if form.validate_on_submit():
        complaint = Compliant(user_category=form.user_category.data,username=form.username.data,
                              compliant=form.comment.data,status="Pending",user_id=current_user.id)
...
def crops():
    ...
    if form.validate_on_submit():
        ...
        crop = Farmer_crops(crop_type=form.crop_type.data,quantity=form.quantity.data,
                image_file=form.picture.data.filename,user_id=current_user.id)
        db.session.add(crop)
...
def status():
    ...
    if data:
        ...
        return redirect(url_for('farm'))
    return render_template('status.html',title="Status of Complaints")
...
def advert():
    ...
    if form.validate_on_submit():
        picture = save_picture(form.picture.data)
        advert = Advert(username=form.username.data,contact=form.contact.data,email=current_user.email,
                Description=form.commodity.data,image_file=form.picture.data.filename,user_id=current_user.id)
...
def advert_customer():
    ...
    if form.validate_on_submit():
        picture = save_picture(form.picture.data)
        advert = Market_available(names=form.names.data,email=form.email.data,
                                  contact=form.contact.data,crop=form.crop.data,
